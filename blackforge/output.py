"""Banner, legal warning, and result formatting for terminal output."""
...
from blackforge.config import AUTHOR, LEGAL_WARNING, TOOL_NAME, VERSION
...
def print_banner() -> None:
    if HAS_PYFIGLET:
        print(f"\033[35m{pyfiglet.figlet_format('BlackForge', font='slant')}\033[0m")
    else:
        print(f"\033[35m\n  {TOOL_NAME} v{VERSION}\n\033[0m")
...
def dump_results(ctx: EngagementContext, output: Optional[str]) -> None:
    ...
    print(f"  Total: {len(ctx.results)} | Success: \033[32m{success}\033[0m"
          f" | Critical: \033[35m{crits}\033[0m\n")
    ...
    for r in ctx.results:
        ...
        print(f"  {icon}{reset} [{r.module}] {r.action}{tgt}")
        if r.notes:
            print(f"        {r.notes}")
    ...
    if output:
        payload = {
            ...
            "credentials": [
                {"type": c.type, "value": c.value, "source": c.source}
                for c in ctx.credentials
            ],
            "loot": ctx.loot,
        }
